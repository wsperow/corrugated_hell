...
@export var KEY_BIND_UP := "ui_move_forward"
@export var KEY_BIND_LEFT := "ui_move_left"
@export var KEY_BIND_RIGHT := "ui_move_right"
@export var KEY_BIND_DOWN := "ui_move_backward"
@export var KEY_BIND_JUMP := "ui_move_jump"
...
@export var TICK_STATS := true
@export var START_FOOD := 80
@export var START_WATER := 80
@export var FOOD_TICK := 400
@export var WATER_TICK := 300
...
#stats
@onready var food = START_FOOD
@onready var water = START_WATER
var food_tick = 0
var water_tick = 0
...
func _process(delta):
	...
	if TICK_STATS:
		...
		if water < 0:
			water = 0
		#do something
		if food_tick == FOOD_TICK:
			food_tick = 0
			food -= 1
		if water_tick == WATER_TICK:
			water_tick = 0
			water -= 1
		water_tick += 1
		food_tick += 1
		ui.update_data(food, water)
...
func give_food(ammount: int):
	food += ammount
	print("Ate food!")

func give_water(ammount: int):
	water += ammount

func give_key(type: String):
	pass

func _on_ray_cast_3d_set_label_visible(visible):
	ui.set_label_visible(visible)


func _on_ray_cast_3d_set_text(text):
	ui.set_text(text)
